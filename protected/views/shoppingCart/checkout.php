...
<script type="text/javascript">
   ...
   // khi chọn đã có tài khoan hay thanh toán với quyền khách hàng
    ...
    function checkBilling(){
        ...
        var fax=$('#fax').val();
        if(first_name.length>0&&last_name.length>0&&company_name.length>0&&email_address.length>0&&IsEmail(email_address)==true &&address.length>0&&postal_code.length>0&& validateZipCode(postal_code)==true &&telephone.length>0&& validatePhone(telephone)==true &&fax.length>0){
             $('#shpping').show();
        }
    }
    ...
    function checkShipping(){
       var first_name_ship=$('#first_name_ship').val();
        ...
        if(first_name_ship.length>0&&last_name_ship.length>0&&company_name_ship.length>0&&email_address_ship.length>0&&IsEmail(email_address_ship)==true &&address_ship.length>0&&postal_code_ship.length>0&&validateZipCode(postal_code_ship)==true&& telephone_ship.length>0&& validatePhone(telephone_ship)==true&&fax_ship.length>0){
            $('#shppingMethod').show();
        } 
    }
    function getValue(){

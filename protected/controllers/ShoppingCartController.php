		...
		if(!empty($_POST)){
			...
			if($modelOrder->save()){
				...
				foreach ($cart as $key => $value) {
					...
					$modelDetail->save();    
                                 $message.= '<tr><td>'.$i.'</td><td>'.$infoProduct->name.'</td><td><img src="http://localhost:8080/kuteshop/'.$infoProduct->image.'" width=100 /></td><td>'.$value.'</td><td>'.$total.'</td></tr>';
                        ...
                        $this-> mailsend(Yii::app()->request->getPost("email_address_ship"),"thuylinh@gmail.com", 'Shopping Cart', $message);
